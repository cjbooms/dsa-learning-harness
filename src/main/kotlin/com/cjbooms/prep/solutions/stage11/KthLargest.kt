 ...
 */
class KthLargest(private val kth: Int) {

    init {
        require(kth > 0) { "k must be positive, was $kth" }
    }

    /**
     * Record a new value from the stream.
     *
     * Structure ritual: maintain a min-heap of size kth. Drop the smallest
     * element from the heap whenever adding a value that exceeds it; that
     * keeps the heap = the current top kth values, smallest of them on top.
     */
    fun add(value: Int) {
        // Step 1: always admit the value. We can always decide to drop it
        // back out once we know whether it belongs in the top k.
        minHeap.add(value)
        // Step 2: if admitting pushed us past size kth, evict the smallest —
        // that's the current (kth+1)-th largest, which cannot be in the top kth.
     ...
     *
     * If fewer than kth values have been added, this returns the smallest
     * value seen so far (the "kth largest" in a pool of fewer than kth items).
    ...
    fun peek(): Int {
        ...
        // Min-heap root is the smallest of the tracked values. Once size == kth
