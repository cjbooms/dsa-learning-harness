...
class ConnectionPool(
    ...
    fun lease(): Connection {
        // 1. Acquire a permit — blocks while N other threads hold leases.
        permits.acquire()
        // 2. Prefer an idle connection so we don't pay the open() cost twice.
        //    pollFirst is lock-free and returns null if the deque is empty.
        return idle.pollFirst() ?: openConnection()
    }

    fun release(conn: Connection) {
        // Return to the idle pool first, then release the permit so a
        // waiting lease() sees both the idle slot and the free permit.
        idle.addLast(conn)
        permits.release()
    }
    ...
    val activeCount: Int
        get() = maxConnections - permits.availablePermits()
